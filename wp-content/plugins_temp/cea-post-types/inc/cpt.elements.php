			...
			if( isset( $posts_shares['Enabled'] ) && !empty( $posts_shares['Enabled'] ) ) { 
				foreach ( $posts_shares['Enabled'] as $key => $value ){
					...
					?>
							...
							<li class="nav-item"><a href="http://www.linkedin.com/shareArticle?mini=true&url=<?php esc_url( get_permalink() ); ?>&title=<?php echo urlencode(get_the_title()); ?>&summary=&source=<?php echo bloginfo('name'); ?>" class="social-linkedin share-linkedin" target="_new"><i class="ti-linkedin"></i></a></li>
					...
					?>
						...
						<li class="nav-item"><a href="http://pinterest.com/pin/create/button/?url=<?php esc_url( get_permalink() ); ?>&amp;media=<?php echo ( ! empty( $image[0] ) ? $image[0] : '' ); ?>&description=<?php echo urlencode(get_the_title()); ?>" class="social-pinterest share-pinterest" target="blank"><i class="ti-pinterest"></i></a></li>
	...
	function ceaCPTNav(){ ?>
		<div class="custom-post-nav">
			...
			<div class="prev-nav-link">
				<a href="<?php echo get_permalink( $prev_post->ID ); ?>" class="ajax-post-link" data-post-id="<?php echo $prev_post->ID; ?>" data-type="<?php echo $prev_post->post_type; ?>"><i class="ti-arrow-left"></i><h5><?php echo esc_html( get_the_title( $prev_post->ID ) ); ?></h5></a>
			</div>
			<?php else: ?>
			...
			<div class="next-nav-link">
				<a href="<?php echo get_permalink( $next_post->ID ); ?>" class="ajax-post-link" data-post-id="<?php echo $next_post->ID; ?>" data-type="<?php echo $next_post->post_type; ?>"><h5><?php echo esc_html( get_the_title( $next_post->ID ) ); ?></h5><i class="ti-arrow-right"></i></a>
			</div>
			<?php else: ?>
