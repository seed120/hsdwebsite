...
class CEA_Elementor_Portfolio_Widget extends Widget_Base {	
	...
	protected function register_controls() {
		$helper_instance = new CEA_Elementor_Portfolio_Widget(); 
		//get authors
		$authors = $helper_instance->cea_get_authors();
		
		//get categories
		$categories = $helper_instance->cea_post_type_taxonomies( 'term_id', 'portfolio-categories' );
		
		//get post titles
		$post_titles = $helper_instance->cea_get_post_titles( 'cea-portfolio' );
		...
		$this->add_control(
			...
			[
				...
				"description"	=> esc_html__( "This is option for portfolio columns on Tablets.", "cea-post-types" ),
				"default"		=> "6",
				"options"		=> [
					"3"			=> esc_html__( "4 Columns", "cea-post-types" ),
					"4"			=> esc_html__( "3 Columns", "cea-post-types" ),
		...
		$this->add_control(
			...
			[
				...
				"description"	=> esc_html__( "This is option for portfolio columns on Mobile.", "cea-post-types" ),
				"default"		=> "12",
				"options"		=> [
					"3"			=> esc_html__( "4 Columns", "cea-post-types" ),
					"4"			=> esc_html__( "3 Columns", "cea-post-types" ),
		...
		$this->add_control(
			...
			[
				...
				"description"	=> esc_html__( "This is option for portfolio load more.", "cea-post-types" ),
				"default"		=> "none",
				"options"		=> [
		...
		$this->add_control(
			...
			[
				...
				"description"	=> esc_html__( "This is option for portfolio image accordion option.", "cea-post-types" ),
				"type" 			=> Controls_Manager::SWITCHER,
				"default" 		=> "no"
			]
		);
		...
		$this->start_controls_tabs( 'tabs_portfolio_category_style' );
		$this->start_controls_tab(
			'tab_portfolio_category_normal',
			[
				'label' => esc_html__( 'Normal', 'cea-post-types' ),
			]
		);
	...
	public function render_content() {
		...
		if( !$portfolio_masonry && !$slide_opt && !$extent_opt ){
		...
		<div class="elementor-widget-container portfolio-wrapper<?php echo esc_attr( $class_names ); ?>" <?php echo ( $extent_opt ) ? 'data-wrap="'. $accordion_break_pts .'"' : ''; ?> >
			<?php 
				if( !$portfolio_masonry && !$slide_opt && !$extent_opt ){
					echo '<div class="row">';
				}
	...
	protected function render() {
		...
		$cols_tab = isset( $portfolio_cols_tab ) ? $portfolio_cols_tab : 12;
		$col_mbl = isset( $portfolio_cols_mbl ) ? $portfolio_cols_mbl : 12;
		...
		if ( $query->have_posts() ) {
			...
			if( $slide_opt ) {
				...
				echo '<div class="isotope' . esc_attr( $isotope_class ) . '" data-gutter="' . esc_attr( $portfolio_gutter ) . '" data-cols="' . esc_attr( 12 / absint( $cols ) ) . '" data-layout="' . esc_attr( $masonry_layout ) . '" data-infinite="' . esc_attr( $portfolio_infinite ) . '" data-lazyload="' . esc_attr( $lazy_load ) . '" >';
			...
			while ( $query->have_posts() ) : $query->the_post();
				...
				$cat_class = '';
				if( $portfolio_masonry && $isotope_filter && $filter_catoutput != '' ){
				...
				if( $extent_opt ) {
					...
					if( $elemetns ) {
						foreach( $elemetns['Enabled'] as $key => $content ) {
							echo $this->cea_portfolio_shortcode_elements( $key, $portfolio_array, $settings);
						}
					}
				...
				if( !$extent_opt ) :
					...
					echo '<div class="'. esc_attr( $col_class . $cat_class ) . ($enable_mouse_hover ? ' cursor-hover-content' : '') .'" '. $this->get_render_attribute_string( 'cea-portfolio-tilt' ) .  $cursor_attr .'>';
			...
			if( !$portfolio_masonry && !$slide_opt && !$extent_opt ){
				if ( $portfolio_load_more !== 'none' ) {
					$element_set = htmlspecialchars(json_encode( $this->get_settings_for_display() ), ENT_QUOTES, 'UTF-8');
					echo '<div id="portfolio-load-more-container"></div>'; // Container for new items
					echo '<div class="d-flex justify-content-center mt-5"><button id="portfolio-load-more" class="load-more-btn btn cpt-load-more '. ($portfolio_load_more == "scroll" ? "load-btn-scroll" : "") .'" data-cpt="cea-portfolio" data-elementor="'. $element_set .'" >' . esc_html__('Load More', 'zozo-portfolio') . '</button></div>';
				}
			}elseif( $slide_opt ) {
