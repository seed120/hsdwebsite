...
class CEA_Elementor_Team_Widget extends Widget_Base {
	...
	public function get_script_depends() {
		return [ 'tilt', 'owl-carousel', 'magnific-popup', 'imagesloaded', 'infinite-scroll', 'isotope', 'cea-custom-front'  ];
	}
	...
	protected function register_controls() {
		...
		//get categories
		$categories = $helper_instance->cea_post_type_taxonomies( 'term_id', 'team-categories' );
		...
		$this->add_control(
			...
			[
				...
				"description"	=> esc_html__( "This is option for team columns on Tablets.", "cea-post-types" ),
				"default"		=> "6",
				"options"		=> [
					"3"			=> esc_html__( "4 Columns", "cea-post-types" ),
					"4"			=> esc_html__( "3 Columns", "cea-post-types" ),
		...
		$this->add_control(
			...
			[
				...
				"description"	=> esc_html__( "This is option for team columns on Mobile.", "cea-post-types" ),
				"default"		=> "12",
				"options"		=> [
					"3"			=> esc_html__( "4 Columns", "cea-post-types" ),
					"4"			=> esc_html__( "3 Columns", "cea-post-types" ),
		...
		$this->add_control(
			...
			[
				...
				"description"	=> esc_html__( "This is option for teams load more.", "cea-post-types" ),
				"default"		=> "none",
				"options"		=> [
		...
		$this->add_control(
			...
			[
				...
				"description"	=> esc_html__( "Enabel to show team filter by category.", "cea-post-types" ),
				"type" 			=> Controls_Manager::SWITCHER,
				"default" 		=> "no",
				"condition" 	=> [
					"team_masonry" => "yes",
		...
		$this->add_control(
			...
			[
				...
				"description"	=> esc_html__( "Here you can mention team's first filter text.", "cea-post-types" ),
			...
			[
				...
				"ddvalues" 			=> [ 
					...
					"disabled"		=> [
						...
						"designation"   => esc_html__( "Designation", "cea_post_types"),
		...
		$this->add_control(
			...
			[
				...
				"description"	=> esc_html__( "This is option for team image accordion option.", "cea-post-types" ),
				"type" 			=> Controls_Manager::SWITCHER,
				"default" 		=> "no"
			]
		);
		...
		$this->add_control(
			...
			$this->start_controls_tabs( 'tabs_team_category_style' );
			$this->start_controls_tab(
				'tab_team_category_normal',
				[
					'label' => esc_html__( 'Normal', 'cea-post-types' ),
				]
			);
		...
		$this->add_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .team-designation .post-designation-head' => 'text-transform: {{VALUE}};'
					],
				]
			);
			$this->start_controls_tabs( 'tabs_team_designation_style' );
			$this->start_controls_tab(
				'tab_team_designation_normal',
				[
					'label' => esc_html__( 'Normal', 'cea-post-types' ),
				]
			);
			$this->add_control(
				...
				[
					...
					'default' => '',
					'selectors' => [
						'{{WRAPPER}} .team-designation .post-designation-head' => 'color: {{VALUE}};', // Adjusted selector
					],
				]
			);
			$this->add_responsive_control(
				'team_designation_scale',
				[
					'label' => esc_html__( 'Scale', 'cea-post-types' ),
					'type' => Controls_Manager::SLIDER,
					'default' => [
			...
			$this->add_control(
				...
				[
					...
					'default' => '',
					'selectors' => [
						'{{WRAPPER}} .team-designation:hover .post-designation-head' => 'color: {{VALUE}};',
					],
				]
			);
			$this->add_responsive_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .team-designation:hover' => 'transform: scale({{SIZE}});'
					],
				]
			);	
			$this->end_controls_tab();
		...
		$this->add_responsive_control(
			...
			[
				...
				"description"	=> esc_html__( "This is option for teams image accordion size.", "cea-post-types" ),
				"default" 		=> "670",
				"selectors"	    => [ 
		...
		$this->add_responsive_control(
			...
			[
				...
				"description"   => esc_html__( "Adjust the teams image width in normal state (in percentage).", "cea-post-types" ),
				"size_units"	=> [ 'px', '%', 'rem', 'em' ],
				"default"       => [
					"size" => 300,
					"unit" => 'px',
		...
		$this->add_responsive_control(
			...
			[
				...
				"description"   => esc_html__( "Adjust the teams image height in normal state (in percentage).", "cea-post-types" ),
				"size_units"	=> [ 'px', '%', 'rem', 'em' ],
				"default"       => [
					"size" => 200,
					"unit" => 'px',
		...
		$this->add_responsive_control(
			...
			[
				"type"          => Controls_Manager::SLIDER,
				"label"         => esc_html__( "Image Width ", "cea-post-types" ),
				"description"   => esc_html__( "Adjust the teams image width in hover state (in percentage).", "cea-post-types" ),
				"size_units"	=> [ 'px', '%', 'rem', 'em' ],
				"default"       => [
					"size" => 600,
					"unit" => 'px',
		...
		$this->add_responsive_control(
			...
			[
				"type"          => Controls_Manager::SLIDER,
				"label"         => esc_html__( "Image Height ", "cea-post-types" ),
				"description"   => esc_html__( "Adjust the teams image height in hover state (in percentage).", "cea-post-types" ),
				"size_units"	=> [ 'px', '%', 'rem', 'em' ],
				"default"       => [
					"size" => 400,
					"unit" => 'px',
	...
	public function render_content() {
		...
		if( !$team_masonry && !$slide_opt && !$extent_opt ){
		...
		<div class="elementor-widget-container team-wrapper<?php echo esc_attr( $class_names ); ?>" <?php echo ( $extent_opt ) ? 'data-wrap="'. $accordion_break_pts .'"' : ''; ?>>
	...
	protected function render() {
		...
		$cols_tab = isset( $team_cols_tab ) ? $team_cols_tab : 12;
		$col_mbl = isset( $team_cols_mbl ) ? $team_cols_mbl : 12;
		...
		if( $isotope_filter && !empty( $include_cats ) ){
			foreach( $include_cats as $index => $cat ){
				if( term_exists( absint( $cat ), 'team-categories' ) ){
					$cat_term = get_term_by( 'id', absint( $cat ), 'team-categories' );	
					if( isset( $cat_term->term_id ) ){
						$filter_catoutput .=  '<li><a href="#" class="isotope-filter-item" data-filter=".team-filter-'. esc_attr( $cat ) .'">'. esc_html( $cat_term->name ) .'</a></li>';	
					}
				}
			}
		}
		...
		if ( $query->have_posts() ) {
			...
			if( $slide_opt ) {
				...
				echo '<div class="isotope' . esc_attr( $isotope_class ) . '" data-gutter="' . esc_attr( $team_gutter ) . '" data-cols="' . esc_attr( 12 / absint( $cols ) ) . '" data-layout="' . esc_attr( $masonry_layout ) . '" data-infinite="' . esc_attr( $team_infinite ) . '" data-lazyload="' . esc_attr( $lazy_load ) . '" >';
			...
			while ( $query->have_posts() ) : $query->the_post();
				...
				if (in_array('category', $mouse_hover_animation)) {
					$categories = get_the_terms(get_the_ID(), 'team-categories');
				...
				$cat_class = '';
				if( $team_masonry && $isotope_filter && $filter_catoutput != '' ){
				...
				if( $extent_opt ) {
					...
					if( $elemetns ) {
						foreach( $elemetns['Enabled'] as $key => $content ) {
							echo $this->cea_team_shortcode_elements( $key, $team_array, $settings);
						}
					}
				...
				if( !$extent_opt ) :
					...
					echo '<div class="'. esc_attr( $col_class . $cat_class ) . ($enable_mouse_hover ? ' cursor-hover-content' : '') .'" '. $this->get_render_attribute_string( 'cea-team-tilt' ) .  $cursor_attr .'>';
			...
			if( !$team_masonry && !$slide_opt && !$extent_opt ){
				if ( $team_load_more !== 'none' ) {
					$element_set = htmlspecialchars(json_encode( $this->get_settings_for_display() ), ENT_QUOTES, 'UTF-8');
					echo '<div id="teams-load-more-container"></div>'; // Container for new items
					echo '<div class="d-flex justify-content-center mt-5"><button id="teams-load-more" class="load-more-btn btn cpt-load-more '. ($team_load_more == "scroll" ? "load-btn-scroll" : "") .'" data-cpt="cea-team" data-elementor="'. $element_set .'">' . esc_html__('Load More', 'cea-post-types') . '</button></div>';
				}
			}elseif( $slide_opt ) {
	...
	function cea_team_shortcode_elements( $element, $opts = array(), $settings = null ){
		...
		switch( $element ){
			...
			case "designation":
				$designation = get_post_meta( get_the_ID(), 'cea_team_designation', true );
				if( $designation ) :
					$output .= '<div class="team-designation">';
						$output .= '<div class="post-designation-head">' . esc_html( $designation ) . '</div>';
