(function ($) {
    ...
    var _body = $('body.page');
    _body.addClass('cea-canvas-effect');
    var _canvasEle = '<div class="cea-canvas-outer"><canvas class="this" id="cea-canvas-fluid"></canvas></div>';
    _body.append(_canvasEle);
    ...
    function initCanvas(el, config) {
        ...
        config = __spreadValues({
            ...
            SUNRAYS_WEIGHT: 1,
            TYPE_COLOR: false,
            COLOR: cea_fluid_settings.color
        }, config);
...
`);
    ...
    function fluidAnimation(reinit = false) {
        if (reinit) {
            jQuery("#cea-canvas-fluid").remove();
            jQuery('.cea-canvas-outer').append("<canvas id='cea-canvas-fluid'></canvas>");
        }

        var canva_ele = $('#cea-canvas-fluid')

        initCanvas(document.getElementById('cea-canvas-fluid'), {
            IMMEDIATE: false, // Whether to trigger multiple random splats when initialized
            TRIGGER: 'hover',
            SIM_RESOLUTION: parseInt(cea_fluid_settings.resolution),
            DYE_RESOLUTION: 512,
            DENSITY_DISSIPATION: parseFloat(cea_fluid_settings.density),
            VELOCITY_DISSIPATION: parseFloat(cea_fluid_settings.velocity),
            PRESSURE: parseFloat(cea_fluid_settings.pressure),
            PRESSURE_ITERATIONS: 20,
            CURL: 5,
            SPLAT_RADIUS: parseFloat(cea_fluid_settings.splat_radius),
            SHADING: true,
            COLORFUL: true,
            PAUSED: false,
            BACK_COLOR: { r: 0, g: 0, b: 0 },
            ...
            FREQ_RANGE_START: 0,
            TRANSPARENT: true,
            TYPE_COLOR: cea_fluid_settings.color_type,
            COLOR: cea_fluid_settings.color,
            BACKSPACE_ANIMATION: cea_fluid_settings.backspace_animation
        });
    }
    ...
    jQuery(window).on('elementor/frontend/init', function () {
        ...
        document.addEventListener('mousemove', event => {
            if (event.isTrusted && newEvent) {
                const canvas = document.getElementById('cea-canvas-fluid');
                if ( canvas ) {
