<?php
/**
 * Zozo Advanced Headers Bar Post Meta Box
 *
 * @package   Zozo Pro
 */
 ...
 * Meta Boxes setup
 */
class Zozo_Target_Rules_Fields {
	/**
	 * Instance
	 *
	 * @since  1.0.0
	...
	public function __construct() {
		define( 'ZOZO_TARGET_RULE_URI', plugins_url( '/', __FILE__ ) );
		define( 'ZOZO_TARGET_RULE_VER', '1.0.0' );
		define( 'ZOZO_TARGET_RULE_DIR', plugin_dir_path( __FILE__ ) );
	...
	public static function get_location_selections() {
		...
		$post_types = apply_filters( 'zozo_location_rule_post_types', array_merge( $post_types, $custom_post_type ) );

		$special_pages = array(
			'special-404'    => __( '404 Page', 'zozo-header-footer' ),
			'special-search' => __( 'Search Page', 'zozo-header-footer' ),
			'special-blog'   => __( 'Blog / Posts Page', 'zozo-header-footer' ),
			'special-front'  => __( 'Front Page', 'zozo-header-footer' ),
			'special-date'   => __( 'Date Archive', 'zozo-header-footer' ),
			'special-author' => __( 'Author Archive', 'zozo-header-footer' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'zozo-header-footer' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'zozo-header-footer' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'zozo-header-footer' ),
					'basic-singulars' => __( 'All Singulars', 'zozo-header-footer' ),
					'basic-archives'  => __( 'All Archives', 'zozo-header-footer' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'zozo-header-footer' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'zozo-header-footer' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc.', 'zozo-header-footer' ),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'zozo_display_on_list', $selection_options );
	}
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'zozo-header-footer' ),
				'value' => array(
					'all'        => __( 'All', 'zozo-header-footer' ),
					'logged-in'  => __( 'Logged In', 'zozo-header-footer' ),
					'logged-out' => __( 'Logged Out', 'zozo-header-footer' ),
				),
			),

			'advanced' => array(	
				'label' => __( 'Advanced', 'zozo-header-footer' ),
				'value' => array(),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'zozo_user_roles_list', $selection_options );
	}
	...
	public function admin_styles() {
		...
		wp_enqueue_style( 'zhf-admin', ZOZO_HF_CORE_URL . 'admin/assets/css/zhf-meta-box.css', false, '1.0.0' );
		
		wp_enqueue_script( 'zozo-select2', ZOZO_TARGET_RULE_URI . 'select2.js', array( 'jquery' ), ZOZO_TARGET_RULE_VER, true );
		...
		if ( '' !== $wp_lang ) {
			...
			if ( isset( $select2_lang[ $wp_lang ] ) && file_exists( ZOZO_TARGET_RULE_DIR . 'i18n/' . $select2_lang[ $wp_lang ] . '.js' ) ) {
				$ast_lang = $select2_lang[ $wp_lang ];
				wp_enqueue_script(
					'zozo-select2-lang',
					ZOZO_TARGET_RULE_URI . 'i18n/' . $select2_lang[ $wp_lang ] . '.js',
					array(
						'jquery',
						'zozo-select2',
					),
					ZOZO_TARGET_RULE_VER,
					true
				);
			}
		}
		...
		wp_register_script(
			'zozo-target-rule',
			ZOZO_TARGET_RULE_URI . 'target-rule.js',
			array(
				'jquery',
				'zozo-select2',
			),
		...
		wp_enqueue_script( 'zozo-target-rule' );

		wp_register_script(
			'zozo-user-role',
			ZOZO_TARGET_RULE_URI . 'user-role.js',
			array(
				'jquery',
			),
			ZOZO_TARGET_RULE_VER,
		...
		wp_enqueue_script( 'zozo-user-role' );

		wp_register_style( 'zozo-select2', ZOZO_TARGET_RULE_URI . 'select2.css', '', ZOZO_TARGET_RULE_VER );
		wp_enqueue_style( 'zozo-select2' );
		wp_register_style( 'zozo-target-rule', ZOZO_TARGET_RULE_URI . 'target-rule.css', '', ZOZO_TARGET_RULE_VER );
		wp_enqueue_style( 'zozo-target-rule' );
		...
		$localize_vars = array(
			'ast_lang'      => $ast_lang,
			'please_enter'  => __( 'Please enter', 'zozo-header-footer' ),
			'please_delete' => __( 'Please delete', 'zozo-header-footer' ),
			'more_char'     => __( 'or more characters', 'zozo-header-footer' ),
			'character'     => __( 'character', 'zozo-header-footer' ),
			'loading'       => __( 'Loading more results…', 'zozo-header-footer' ),
			'only_select'   => __( 'You can only select', 'zozo-header-footer' ),
			'item'          => __( 'item', 'zozo-header-footer' ),
			'char_s'        => __( 's', 'zozo-header-footer' ),
			'no_result'     => __( 'No results found', 'zozo-header-footer' ),
			'searching'     => __( 'Searching…', 'zozo-header-footer' ),
			'not_loader'    => __( 'The results could not be loaded.', 'zozo-header-footer' ),
			'search'        => __( 'Search pages / post / categories', 'zozo-header-footer' ),
			'ajax_nonce'    => wp_create_nonce( 'zhf-get-posts-by-query' ),
		);
		wp_localize_script( 'zozo-select2', 'astRules', $localize_vars );
	}
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'zozo-header-footer' );
		$saved_values   = $value;
		$output         = '';
		...
		/* WP Template Format */
		$output .= '<script type="text/html" id="tmpl-zozo-target-rule-' . $rule_type . '-condition">';
		$output .= '<div class="zozo-target-rule-condition ast-target-rule-{{data.id}}" data-rule="{{data.id}}" >';
		...
		$output .= '<option value="">' . __( 'Select', 'zozo-header-footer' ) . '</option>';
		...
		$output .= '</div> <!-- zozo-target-rule-condition -->';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'zozo-header-footer' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'zozo-header-footer' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'zozo-header-footer' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			$output .= '<div class="zozo-target-rule-condition ast-target-rule-' . $index . '" data-rule="' . $index . '" >';
			/* Condition Selection */
			$output .= '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
			$output .= '<div class="target_rule-condition-wrap" >';
		...
		if ( 'display' == $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'zozo-header-footer' ) . '</a>';
			$output .= '</div>';
		}
	...
	public function get_current_layout( $layout_id, $option ) {
		...
		$current_layout = apply_filters( 'zozo_target_page_settings', $current_layout, $layout_id );
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		/* WP Template Format */
		$output         .= '<script type="text/html" id="tmpl-zozo-user-role-condition">';
			$output     .= '<div class="zozo-user-role-condition ast-user-role-{{data.id}}" data-rule="{{data.id}}" >';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			$output .= '</optgroup>';
					...
					$output .= '</select>';
				$output     .= '</div>';
			$output         .= '</div> <!-- zozo-user-role-condition -->';
		$output             .= '</script>';
		...
		foreach ( $saved_values as $index => $data ) {
			$output     .= '<div class="zozo-user-role-condition ast-user-role-' . $index . '" data-rule="' . $index . '" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		if ( is_array( self::$current_page_data ) && isset( self::$current_page_data[ $post_type ] ) ) {
			return apply_filters( 'zozo_get_display_posts_by_conditions', self::$current_page_data[ $post_type ], $post_type );
		}
		...
		if ( ! empty( $already_set_rule ) ) {
			add_action(
				...
				function() use ( $already_set_rule ) {
					...
					$notice = sprintf( __( 'The same display setting is already exist in %s post/s.', 'zozo-header-footer' ), $rule_set_titles );
 ...
 */
Zozo_Target_Rules_Fields::get_instance();
